...
/* --- 
   ...
   --- 
*/
$sqlSelectnhacungcap = <<<EOT
    SELECT ncc.ncc_ma, ncc.ncc_ten, COUNT(*) soluongsanpham
    FROM `nhacungcap`ncc
    LEFT JOIN `sanpham` sp ON ncc.ncc_ma = sp.ncc_ma
    GROUP BY ncc.ncc_ma, ncc.ncc_ten
EOT;

// Thực thi câu truy vấn SQL để lấy về dữ liệu ban đầu của record 
$resultSelectnhacungcap = mysqli_query($conn, $sqlSelectnhacungcap);
...
$nhacungcapData = [];
while ($row = mysqli_fetch_array($resultSelectnhacungcap, MYSQLI_ASSOC)) {
    $nhacungcapData[] = array(
        'ncc_ma' => $row['ncc_ma'],
        'ncc_ten' => $row['ncc_ten'],
        'soluongsanpham' => $row['soluongsanpham'],
    );
...
$keyword_nhacungcap = isset($_GET['keyword_nhacungcap']) ? $_GET['keyword_nhacungcap'] : [];
...
$sqlDanhSachSanPham = <<<EOT
    ...
    LEFT JOIN `nhacungcap` ncc ON sp.ncc_ma = ncc.ncc_ma
...
if (!empty($keyword_nhacungcap)) {
    $value = implode(',', $keyword_nhacungcap);
    $sqlWhereArr[] = "ncc.ncc_ma IN ($value)";
}
// Tìm theo khuyến mãi
...
echo $twig->render(
    ...
    [
        ...
        'danhsachnhacungcap' => $nhacungcapData,
        ...
        'keyword_nhacungcap' => $keyword_nhacungcap,
